...
export default function FileUploader(props) {
    ...
    const { getRootProps, getInputProps } = useDropzone({ onDrop, maxFiles: props.isMultiple ? 0 : 1 });

    return (
        <div>
            <input {...getInputProps()}></input>
            {props.isMultiple ? (
                <div className="dropzone file-upload-center file-upload-pointer" {...getRootProps()}>
                    <div>
                ...
                <i className="dripicons-cloud-upload file-upload-pointer" {...getRootProps()} />
            )}
        </div>
    );
}
